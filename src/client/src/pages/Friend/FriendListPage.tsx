...
const FriendListPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`http://127.0.0.1:5005/api/friendslist/?user_id=${userId}`)
      .then((response) => {
        if (response.data) {
  ...
  const addFriend = (friendId: number, nickname: string) => {
    ...
    axios
      .post('http://127.0.0.1:5005/api/friendslist/', {
 ...
 // delete a friend
  const deleteFriend = () => {
    ...
    if (friendToDelete) {
      axios
        .delete(`http://127.0.0.1:5005/api/friendslist/${friendToDelete.list_id}`)
        .then(() => {
          setFriends((prev) =>
